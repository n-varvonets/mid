...
| **Тип алгоритма**   | **Алгоритм**               | **Временная сложность** | **Пространственная сложность** | **Описание**                                                                                 |
|---------------------|----------------------------|-------------------------|---------------------------------|----------------------------------------------------------------------------------------------|
| **Сортировка**      | Пузырьковая сортировка     | \(O(n^2)\)              | \(O(1)\)                        | Простой алгоритм, неэффективен для больших массивов.                                         |
| **Сортировка**      | Сортировка выбором         | \(O(n^2)\)              | \(O(1)\)                        | Ненамного лучше пузырьковой сортировки, но тоже неэффективен для больших данных.             |
| **Сортировка**      | Сортировка вставками       | \(O(n^2)\) / \(O(n)\)   | \(O(1)\)                        | Хорошо работает на почти отсортированных данных.                                             |
| **Сортировка**      | Сортировка слиянием        | \(O(n \log n)\)         | \(O(n)\)                        | Стабильный алгоритм, но требует дополнительной памяти.                                       |
| **Сортировка**      | Быстрая сортировка         | \(O(n \log n)\)         | \(O(\log n)\)                   | Очень быстрый алгоритм на практике, особенно на случайных данных.                            |
| **Сортировка**      | Сортировка кучей           | \(O(n \log n)\)         | \(O(1)\)                        | Не требует дополнительной памяти, но не является стабильным.                                 |
| **Сортировка**      | Сортировка подсчётом       | \(O(n + k)\)            | \(O(n + k)\)                    | Подходит для целочисленных данных с ограниченным диапазоном значений.                        |
| **Сортировка**      | Цифровая сортировка        | \(O(n \cdot k)\)        | \(O(n + k)\)                    | Эффективен для сортировки целых чисел или строк фиксированной длины.                         |
| **Поиск**           | Линейный поиск             | \(O(n)\)                | \(O(1)\)                        | Простой метод поиска, перебирает все элементы.                                               |
| **Поиск**           | Бинарный поиск             | \(O(\log n)\)           | \(O(1)\)                        | Очень эффективен для поиска в отсортированных массивах.                                      |
| **Поиск**           | Интерполяционный поиск     | \(O(\log \log n)\) / \(O(n)\) | \(O(1)\)                | Улучшение бинарного поиска для данных с равномерным распределением.                          |
| **Поиск**           | Экспоненциальный поиск     | \(O(\log n)\)           | \(O(1)\)                        | Используется для поиска в неотсортированных данных, ищет интервал, где может быть элемент.   |
